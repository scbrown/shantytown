...
THE EDITABLE ERA (aegis-ttlr, 2026-07-20 22:15). `st` is now `pipx install -e` —
...
#      a personal worktree (the aegis-daoh incident, replayed in the editable
...
def check_self(*, run=_default_run, canonical: str | None = None,
    ...
    # CHECKOUT itself current? This is aegis-ttlr's original defect — a fix on
...
def _behind_upstream(canonical: str, run) -> tuple[int, str] | None:
    ...
    THE QUESTION THAT MOVED (aegis-ttlr). Under a non-editable install the drift
