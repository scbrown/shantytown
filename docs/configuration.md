...
| `SHANTY_CREATE_JEV_COMMAND` | quoted argv for Camayoc's Jev MCP server, used by agent-only create duplicate advisories; missing configuration produces a visible skip | — |
| `SHANTY_CREATE_ADVISORY_LOG` | JSONL outcome/latency log for create advisories; contains hashes and candidate IDs, not proposed title/body text | `~/.local/state/shantytown/create-advisory.jsonl` |
| `SHANTY_ADVISORY_PRIOR` | internal hook-chain handoff: existing PreToolUse JSON envelope whose context and permission fields the advisory preserves; not a persistent setting | — |
| `SHANTY_BR_BIN` | the `br` executable used by the br tracker backend. May be an installed shared-board transport that preserves the `br` CLI contract (including exit status), or a local executable. Declare it with `SHANTY_BACKEND="br"` in `[env]` so every command uses the same board. | `br` |
...
| `SHANTY_PANE_MASKER` | Fleet `mask-secrets.py` library for pane advice; missing or failed masking refuses inference. | `$XDG_DATA_HOME/aegis-exec-src/scripts/mask-secrets.py` |
| `SHANTY_JEV_CLIENT` | Canonical Camayoc `scripts/jev.py` file for the explicit `crew --pane-state` advisory. Missing client/key fails loudly; no inference is used by dispatch. | `$XDG_DATA_HOME/camayoc-src/camayoc/scripts/jev.py` (XDG defaults to `~/.local/share`) |
...
| `SHANTY_ADMIN_ARTIFACT_GUARD` | Deployment publication guard for administrator Claude `Artifact` calls; exit 2 refuses publication. Unset emits no hook. | — |
