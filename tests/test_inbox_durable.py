"""st inbox --durable — must-survive messages. shantytown #7 (aegis-qdal.qdal.2).
...
class _RecordingInbox:
    ...
    def deliver(self, to, body, frm=None):
        ...
        return Message(id="aegis-dur1", to=to, body=body, frm=frm)
...
def test_durable_persists_when_recipient_is_down(tmp_path, monkeypatch):
    ...
    rc = main(["--root", str(_root(tmp_path)), "inbox", "-d", "ian", "HANDOFF: finish qdal.2"])
    assert rc == OK
    assert len(box.delivered) == 1
    to, body, _frm = box.delivered[0]
    assert body == "HANDOFF: finish qdal.2"
    assert to == "ian"
