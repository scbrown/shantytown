"""role set EMITS the settings.json — the content st agent new's --settings reads.
...
st agent new then finds the settings it refused for a moment ago.
"""
...
def test_every_role_pre_answers_the_project_mcp_prompt():
    ...
    BLOCKS the ready UI: st agent new then reports could-not-tell for a healthy agent
...
# --- THE LOOP CLOSES: role set then st agent new no longer refuses on settings ---------

def test_role_set_then_new_no_longer_refuses_for_missing_settings(tmp_path, monkeypatch):
    """Before #6, st agent new REFUSED a worker with no settings file (the invariant).
    After role set emits it, compose materializes and st agent new proceeds to launch +
    verify. We only assert it got PAST the settings refusal — the pane shows the
    ready banner so verify returns 0."""
    root = _world(tmp_path)
    assert main(["--root", str(root), "roles", "set", "ellie", "worker"]) == OK
    # st agent new: a live pane with the ready banner -> verify 0. If settings were still
    ...
    assert rc == OK, "role set emitted settings but st agent new still refused"
    assert panes.sent, "st agent new should have launched now that settings exist"
