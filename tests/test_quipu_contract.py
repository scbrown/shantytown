...
aegis-0as8. The rest of the quipu tests use doubles: `Recorder` records that
...
def _entity_retract(server: str, name: str) -> None:
    ...
    (triple-level no-ops, aegis-vqy9). Used for teardown so a test leaves the
...
@pytest.mark.xfail(
    reason="aegis-vqy9: quipu triple-level /retract removes nothing, so a "
...
def test_reparent_takes_effect_in_the_real_graph(reg):
    """The observable effect that is currently IMPOSSIBLE (aegis-vqy9): move an
