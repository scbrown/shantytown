...
def test_threshold_reached_NUDGES_THE_AGENT_not_the_coordinator(tmp_path):
    ...
    assert mk().sweep(reg.all().exact()) == {"nudged": [], "escalated": []}   # episode starts
    clock["t"] += 16 * 60
    ...
    assert "st work defer aegis-u140" in msg          # ...close if done, structured DEFER if blocked
...
def test_still_frozen_after_the_nudge_ESCALATES_to_the_coordinator(tmp_path):
    ...
    reg, panes, clock, mk = _world(tmp_path)
    mk().sweep(reg.all().exact())
    clock["t"] += 16 * 60
    assert mk().sweep(reg.all().exact())["nudged"] == ["weaver"]     # nudged the agent
    clock["t"] += 16 * 60                                    # still frozen a window later
    assert mk().sweep(reg.all().exact()) == {"nudged": [], "escalated": ["weaver"]}
...
def test_progress_after_the_nudge_PREVENTS_escalation(tmp_path):
    ...
    mk().sweep(reg.all().exact())
    clock["t"] += 16 * 60
    assert mk().sweep(reg.all().exact())["nudged"] == ["weaver"]
    panes.screens["p-weaver"] = IDLE + "\n  (agent acted on the nudge)"  # progress
    clock["t"] += 16 * 60
    r = mk().sweep(reg.all().exact())
...
def test_a_decision_labeled_anchor_is_NEVER_nudged(tmp_path):
    ...
    assert mk().sweep(reg.all().exact()) == {"nudged": [], "escalated": []}
...
def test_a_changing_pane_is_progress(tmp_path):
    ...
    clock["t"] += 16 * 60
    panes.screens["p-weaver"] = IDLE + "\n  new output line"
    assert mk().sweep(reg.all().exact()) == {"nudged": [], "escalated": []}  # fresh episode
    assert panes.sent == []
...
def test_bd_hiccup_fails_open(tmp_path):
    ...
    assert a.sweep(reg.all().exact()) == {"nudged": [], "escalated": []} and panes.sent == []
...
def test_undelivered_escalation_does_not_burn_the_stage(tmp_path):
    ...
    mk().sweep(reg.all().exact())
    clock["t"] += 16 * 60
    assert mk().sweep(reg.all().exact())["nudged"] == ["weaver"]   # stage 1 delivered
