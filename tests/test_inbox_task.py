"""st inbox is send-keys. st task creates. Both must be able to REFUSE."""
import json
...
def test_inbox_is_send_keys_and_nothing_else(tmp_path, monkeypatch):
    ...
    rc = main(["--root", str(_root(tmp_path)), "inbox", "ian", "go", "read", "st-1"])
    assert rc == OK
    assert sent == [("%1", "go read st-1")]


def test_inbox_refuses_unknown_agent(tmp_path):
    assert main(["--root", str(_root(tmp_path)), "inbox", "nobody", "hi"]) == REFUSED


def test_inbox_refuses_agent_with_no_pane(tmp_path):
    assert main(["--root", str(_root(tmp_path)), "inbox", "nopane", "hi"]) == REFUSED


def test_inbox_cannot_tell_when_pane_is_gone(tmp_path, monkeypatch):
    ...
    assert main(["--root", str(_root(tmp_path)), "inbox", "ian", "hi"]) == CANNOT_TELL


def test_inbox_dry_run_sends_nothing(tmp_path, monkeypatch):
    import shantytown.cli as cli
    class Boom:
    ...
    assert main(["--root", str(_root(tmp_path)), "inbox", "ian", "-n", "hi"]) == OK
...
def test_inbox_refuses_a_send_flag_with_nothing_to_send(tmp_path):
    """`st inbox -d ian` is a typo, not a request to READ ian's inbox. The read
    modes take no message, so the ambiguity is real and is refused rather than
    silently resolved into the other command."""
    assert main(["--root", str(_root(tmp_path)), "inbox", "-d", "ian"]) == REFUSED
