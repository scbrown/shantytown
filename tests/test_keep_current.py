"""Keep Current, mechanized (aegis-4zld): st pulls the agent's workspace at the
...
def test_local_tracked_dirt_refuses_loudly_and_never_forces(repo_pair):
    """The aegis-43ph condition: diverged tracked content. ff-only refuses;
    the local work SURVIVES (never force/reset — aegis-repg/iaef)."""
    _origin, clone = repo_pair
