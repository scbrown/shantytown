...
def test_canonical_source_from_a_linked_worktree_names_the_MAIN_checkout(monkeypatch):
    """THE HOLE THAT SURVIVED THE EDITABLE ERA (aegis-daoh, measured 2026-07-23).
...
# --- the editable era (aegis-ttlr): the checkout IS the install ----------
...
def test_a_checkout_BEHIND_its_upstream_is_BROKEN_not_green(tmp_path):
    """aegis-ttlr's original defect, the layer every other row waves through: a
